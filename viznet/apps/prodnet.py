...
def draw_prodnet(ax, num_node_visible):
    ...
    for i in [-1,num_node_visible]:
        handler.add_node('$\sigma_%d$'%(i+1), (i-num_node_visible/2.+0.5,0), kind='invisible', radius=0.3, show_name=False)
    for i in range(num_node_visible):
        A = '$nn_%d$'%(2*i+1)
        B = '$nn_%d$'%(2*i+2)
        with handler.context('fontsize',10):
            handler.add_node(A, (i-num_node_visible/2.,1), kind='basic', radius=0.15, show_name=r'x')
            handler.add_node(B, (i-num_node_visible/2.+0.5,1), kind='basic', radius=0.15, show_name=r'x')
    ...
    handler.text_node_sequence(r'+', text_list=[r'$\sinh$'], offset=(-1.2,-0.2))
