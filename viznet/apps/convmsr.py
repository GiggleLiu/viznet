...
def draw_conv_msr(ax, num_node_visible):
    ...
    handler.text_node_sequence(r'\theta', text_list=[r'$\theta$'], offset=(-0.2,0))
    handler.text_node_sequence(r'even', text_list=[r'$\sigma_%d$'%i for i in range(1, num_node_visible,2)])
    handler.text_node_sequence(r'odd', text_list=[r'$\sigma_%d$'%i for i in range(2, num_node_visible+1,2)])
